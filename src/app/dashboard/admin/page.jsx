...
async function getOverview() {
  try {
    ...
    try {
      ...
      const activeAdminCount = await adminsCollection.countDocuments({
        status: { $ne: 'revoked' },
      });

      if (activeAdminCount === 0 && ObjectId.isValid(user.id)) {
        const authUser = await authDb.collection('user').findOne({ _id: new ObjectId(user.id) });
        ...
        if (authUser) {
          await adminsCollection.insertOne({
            ...
            previousRole: normalizeRole(authUser.role),
            status: 'active',
            isPrime: true,
      ...
      const adminCount = await adminsCollection.countDocuments({ status: 'active' });
      const primeAdmin = await adminsCollection.findOne({
        status: 'active',
        isPrime: true,
      });
      ...
      return {
        ...
        totalRevenue,
        adminCount,
        primeAdminName: primeAdmin?.name || primeAdmin?.email || '',
      };
    } finally {
